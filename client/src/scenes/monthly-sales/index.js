...
const MonthlySales = () => {
  ...
  let monthlySales = []
  if (tmp && tmp.length > 0) {
    monthlySales = tmp.filter((obj) => {
      const today = new Date()
      const saleDate = new Date(obj.updatedAt)
      if (saleDate.getUTCMonth() === today.getUTCMonth()) {
        if(saleDate.getUTCFullYear() === today.getUTCFullYear()){
          return obj
        }
      }
    });
  ...
  return (
    <Box m="1.5rem 2.5rem">
      <FlexBetween>
        <Header title="Monthly Sale" subtitle="List of Monthly Sale" />
      ...
      >
        <DataGrid
          autoHeight
          loading={isLoading || !monthlySales}
